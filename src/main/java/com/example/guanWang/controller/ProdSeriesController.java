...
public class ProdSeriesController {
    ...
    public CommonResult<CommonPage<SeriesModel>> list(@RequestParam(value = "keyword", required = false) String keyword,
        ...
        List<SeriesModel> roleList = prodSeriesService.list(keyword, pageSize, pageNum);
    ...
    public CommonResult update(@PathVariable Long id, @RequestBody SeriesModel seriesModel) {
        int count = prodSeriesService.update(id, seriesModel);
        if (count > 0) {
            return CommonResult.success(count);
        }
        return CommonResult.failed();
    ...
    public CommonResult delete(@PathVariable Long id) {
        int count = prodSeriesService.delete(id);
        if (count > 0) {
            return CommonResult.success(count);
        }
        return CommonResult.failed();
